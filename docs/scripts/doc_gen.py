"""
Iterate the whole module and extract annotations or comments from `BaseModel` class,
then write them into a markdown file.
...
def doc_gen(package_name: str, author: str, file: str):
    ...
    lines = ["# Zerolan Data Documentation\n\n",
             f"Generated from the project `{package_name}` by **{author}** at `{_get_time_iso_string()}`.\n\n"]
    ...
    with open(file, "w+", encoding='utf-8') as f:
        f.writelines(lines)
        print(f"Documentation was saved at: {os.path.abspath(file)}")
